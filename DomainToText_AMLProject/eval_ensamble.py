...
def analyse(checkpoints,target):

    print(args)
    target_txt = args.path_to_txt + '/' + target + '.txt'
    ...
    G1.load_state_dict(torch.load('DomainToText_AMLProject/outputs/SingleSource_'+sources[0] + '/G.pkl'))
    C1.load_state_dict(torch.load('DomainToText_AMLProject/outputs/SingleSource_'+sources[0] + '/C.pkl'))
    ...
    G2.load_state_dict(torch.load('DomainToText_AMLProject/outputs/SingleSource_'+sources[1] + '/G.pkl'))
    C2.load_state_dict(torch.load('DomainToText_AMLProject/outputs/SingleSource_'+sources[1] + '/C.pkl'))
    ...
    G3.load_state_dict(torch.load('DomainToText_AMLProject/outputs/SingleSource_'+sources[2] + '/G.pkl'))
    C3.load_state_dict(torch.load('DomainToText_AMLProject/outputs/SingleSource_'+sources[2] + '/C.pkl'))
    ...
    sources_domain_embeddings_barycenter = np.zeros((3, vec_dim))
    ...
    for n_source, source in enumerate(sources):
        ...
        with torch.no_grad():
            ...
            bary_encoder = resnet101_texture_model.lang_encoder(source)
            sources_domain_embeddings_barycenter[n_source] += bary_encoder.cpu().numpy()[0]
            sources_domain_embeddings_barycenter[n_source] = normalize(sources_domain_embeddings_barycenter[n_source])


    correct_mean = 0
    correct_visual_domain_embedding = 0
    correct_texture_domain_embedding = 0
    correct_texture_domain_embedding_bary = 0
    correct_s1 = 0
    correct_s2 = 0
    correct_s3 = 0
    ...
    count = 0
    print('Evaluation on the Target domain - %s' % (target))
    with torch.no_grad():
        for it, (img, label) in enumerate(tqdm(dataset_test_target)):
            ...
            correct_s1 += pred1.eq(label.data).cpu().sum()
            pred2 = out2.data.max(1)[1]
            correct_s2 += pred2.eq(label.data).cpu().sum()
            pred3 = out3.data.max(1)[1]
            correct_s3 += pred3.eq(label.data).cpu().sum()

            ####### barycenter
            b = normalize(target_domain_embedding_textual[0].cpu().numpy())
            a1 = sources_domain_embeddings_barycenter[0]
            a2 = sources_domain_embeddings_barycenter[1]
            a3 = sources_domain_embeddings_barycenter[2]
            w1_text = cosine_sim(a1, b)
            w2_text = cosine_sim(a2, b)
            w3_text = cosine_sim(a3, b)
            pred_only_texture_domain_embedding_bary = ((w1_text * out1 + w2_text * out2 + w3_text * out3) / (w1_text + w2_text + w3_text)).data.max(1)[1]
            correct_texture_domain_embedding_bary += pred_only_texture_domain_embedding_bary.eq(label.data).cpu().sum()
    ...
    acc_only_s1 = float(correct_s1.item() / len(dataset_test_target.dataset)) * 100
    acc_only_s2 = float(correct_s2.item() / len(dataset_test_target.dataset)) * 100
    acc_only_s3 = float(correct_s3.item() / len(dataset_test_target.dataset)) * 100

    acc_only_texture_domain_embedding_bary = float(correct_texture_domain_embedding_bary.item() / len(dataset_test_target.dataset)) * 100
    print('Accuracy text_domain_embedding_bary: %.2f ' % (acc_only_texture_domain_embedding_bary))

    outp = [acc_only_texture_domain_embedding, acc_only_s1, acc_only_s2, acc_only_s3,acc_only_texture_domain_embedding_bary]
    return outp


def analyse2(checkpoints,target):
    ...
    #embedding of target
    target_domain_embedding_textual = np.zeros((3, vec_dim))
    with torch.no_grad():
        for it, (img, label) in enumerate(tqdm(dataset_test_target)):
            ...
            # textual target embedding -- whole img
            target_embedding = resnet101_texture_model.img_encoder(img)
            target_domain_embedding_textual += target_embedding.cpu().numpy()[0]



        b = normalize(target_domain_embedding_textual / len(dataset_test_target.dataset))
    ...
    outp = [acc_only_texture_domain_embedding, acc_only_s1, acc_only_s2, acc_only_s3]
    return outp
...
def analyse2(checkpoints,target):
    ...
    #embedding of target
    target_domain_embedding_textual = np.zeros(vec_dim)
    with torch.no_grad():
        for it, (img, label) in enumerate(tqdm(dataset_test_target)):
