...
def train():
    ...
    print('PARAMETER : MARGIN ,', cfg.LOSS.MARGIN)
    print('PARAMETER : HARD ,', cfg.HARD_SAMPLES_TRIPLET_LOSS)
    ...
    dataset = TripletTrainData(split=cfg.TRAIN_SPLIT, neg_img=cfg.LOSS.IMG_SENT_WEIGHTS[0] > 0,
                               ...
                               hard_samples=cfg.HARD_SAMPLES_TRIPLET_LOSS, neg_margin=cfg.LOSS.MARGIN, img_feats=cfg.MODEL.IMG_FEATS,
                               lang_encoder_method=cfg.MODEL.LANG_ENCODER, word_encoder=word_encoder, model = model)
    ...
    if len(cfg.LOAD_WEIGHTS) > 0:
        print("Load weights, ", cfg.LOAD_WEIGHTS)
