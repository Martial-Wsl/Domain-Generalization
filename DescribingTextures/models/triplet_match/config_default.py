...
C.LOAD_WEIGHTS = ''
#C.LOAD_WEIGHTS = 'texture_model/BEST_checkpoint.pth'
C.INIT_WORD_EMBED = 'fast_text'  # rand / fast_text
C.LANG_INPUT = 'description'  # description
C.HARD_SAMPLES_TRIPLET_LOSS = True
...
C.LOSS.IMG_SENT_WEIGHTS = (0.5, 1.5)

# config for training
C.TRAIN = CN()
C.TRAIN.TUNE_RESNET = True
C.TRAIN.TUNE_LANG_ENCODER = False
